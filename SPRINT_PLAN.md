...
### Phase 30i — Auto-Generate Default Bet-Graphs ☑
**Root cause (deeper):** Users never create bet-graphs before creating sessions. The `bet_graphs` table is genuinely EMPTY for all modes, so snapshotForMode always returns `[]`.

**Fundamental fix (4 layers):**
- ☑ **Auto-generate on mode create:** `autoGenerateDefaultGraphs()` in `src/lib/server/modes/autoGraphs.ts` — when a mode is created with trackables, auto-generates a "Champion" (Wer hat am meisten?) bet-graph for each entity-scoped trackable.
- ☑ **Auto-generate on mode save:** Same function is called when a mode is saved and still has 0 graphs (catches existing modes that were created before this fix).
- ☑ **Auto-generate on syncBetGraphs:** The `?/syncBetGraphs` action in the round page now calls `autoGenerateDefaultGraphs` before snapshotting — so existing sessions can recover by pressing one button.
- ☑ **Auto-generate on createRound:** Same flow — if session has no graphs, auto-generate for the mode first, then snapshot and instantiate.
