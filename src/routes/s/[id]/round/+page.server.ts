...
import { autoGenerateDefaultGraphs } from '$lib/server/modes/autoGraphs';
...
export const actions: Actions = {
	createRound: async ({ locals, params }) => {
		...
		try {
			...
			if (sess && (sess.betGraphsSnapshot ?? []).length === 0) {
				// Auto-generate default graphs for the mode if it has none
				const mode = await findModeById(sess.modeId);
				if (mode) {
					await autoGenerateDefaultGraphs(mode.id, mode.trackables ?? []);
				}
	...
	syncBetGraphs: async ({ locals, params }) => {
		...
		try {
			...
			// If the mode has 0 bet-graphs, auto-generate defaults first
			const mode = await findModeById(session.modeId);
			if (mode) {
				await autoGenerateDefaultGraphs(mode.id, mode.trackables ?? []);
			}
