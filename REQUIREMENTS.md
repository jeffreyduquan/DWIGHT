...
- **REQ-DRINK-006** Confirmation rule per Mode (`confirmationMode`): `GM` (1 GM signoff) or `PEERS` (N distinct signoffs, `peerConfirmationsRequired`). In `PEERS` mode a GM confirmation counts as a peer. The legacy `EITHER` value behaves like `PEERS` for backward compatibility.
