...
- **REQ-MODE-001** Every Session is bound to exactly one Mode at any given time. The Mode is set at create-time and may be switched by the Host mid-session under the constraints of REQ-MODE-015.
...
- **REQ-MODE-007** A Mode owns zero or more **Bet-Graphs** (table `bet_graphs`): visual node-graph definitions (`{version:1, nodes:[], edges:[]}`) that compile to runtime markets. At session-create the snapshot is copied to `sessions.bet_graphs_snapshot`; if the snapshot is still empty at round-creation time, an **auto-sync** re-snapshots from the mode before instantiating markets, ensuring bet-graphs added after session creation are picked up. At round-betting-open the compiler emits one market per supported outcome via `compileSessionGraphs` and spawns them. Unsupported shapes are skipped (no hard fail). Supported in Phase 6: Wett-Familien A (race, N=1 via `log_rank`), B (arg_max), C (sum+compare), D (count+compare). Other families return `ok:false` and are skipped.
...
- **REQ-MODE-015** **Mode switching within a session.** The Host (GM) can switch the active Mode of a running Session under these constraints:
  1. **No active round.** All rounds must be `SETTLED` or `CANCELLED` (or no rounds exist). If a non-terminal round exists, the switch is rejected.
  2. **Atomic re-snapshot.** On switch, the server atomically updates: `sessions.modeId` → new mode, `sessions.trackables` → re-snapshot from new mode, `sessions.betGraphsSnapshot` → re-snapshot from new mode's bet-graphs, and `config.entityOverrides` → cleared (old entity names no longer apply).
  3. **Entity replacement.** All existing `entities` rows for the session are deleted and re-created from the new mode's `defaultEntities`.
  4. **Player preservation.** `session_players` (userId, role, moneyBalance, betLocked, joinedAt) are untouched. All players keep their current balances, roles, and lock state.
  5. **History preservation.** Already-settled rounds, markets, bets, and drinks remain intact in the DB. They reference entity/trackable IDs from the previous mode but are read-only historical records.
  6. **SSE broadcast.** A `mode_switched` event is emitted to all session subscribers so clients can invalidate and reload.
  7. **UI.** The lobby (`/s/:id`) surfaces a "Mode wechseln" option in the GM section when no round is active. The UI shows a Mode picker (same as `/s/create`) and a confirmation modal warning that entities and bet-graphs will be replaced.
...
- **REQ-UI-069** **Phase 27a Mode-Page Spacing.** Auf `/modes/[id]/+page.svelte` und `/modes/new/+page.svelte` wandert das `pb-24` (Buffer für die fixe Save-Bar) vom `<form class="space-y-5 pb-24">` (ModeForm) ans äußere Page-Container (`<div class="… pt-8 pb-24">`). Dadurch entfällt die große Lücke zwischen dem Mode-Form (Trackables/Entitäten/etc.) und folgenden Sektionen (Wetten, Delete).
- **REQ-UI-070** **Phase 27b Graph-Canvas leer.** Im `SlotGraphEditor` bekommt `.canvas-scroll` die Modifier-Klasse `.empty` (gesetzt wenn `graph.nodes.length === 0`). Diese setzt `overflow-x: hidden` und zwingt `.canvas-grid` auf `width: 100% !important; min-width: 100%`. Folge: bei leerem Graph erscheint keine horizontale Scrollbar — der Canvas-Bereich passt sich der Viewport-Breite an. Sobald Knoten existieren, gilt wieder das berechnete `visibleCols * SLOT_W` Layout mit X-Scroll bei Bedarf.
- **REQ-UI-071** **Phase 27c Catalog/Inspector als Drawer (alle Breakpoints).** Catalog (Bausteine) und Inspector (Eigenschaften) sind nicht mehr permanente Spalten/Overlays, sondern echte Drawer auf allen Viewports. `.editor-root grid-template-columns: 1fr` (nur Canvas + Statusbar), `.catalog` ist `position: absolute; transform: translateX(-100%)` und wird durch `.mobile-open` eingeblendet (analog zu Inspector aus 26a). Die Toolbar-Buttons "Bausteine" und "Eigenschaften" in der Statusbar (mobile-only-Klasse, jetzt immer sichtbar) öffnen die Drawer. Damit ist der Canvas auf Desktop und Mobile gleichermaßen unblockiert.
- **REQ-UI-072** **Phase 27d Template-Wizard trackable-first.** Der Template-Picker auf `/modes/[id]/+page.svelte` ist ein 2-Step-Wizard: Step 1 wählt das Trackable (Event) mit visueller Scope-Badge ("einzel" oder "global"). Step 2 zeigt nur die Templates, die zum gewählten Scope passen, mit **personalisierten Titeln** (`personalizedTitle()`), z.B. "Wer macht zuerst „Tore"?", "Top-N nach „Punkten"". Step 3 (Form) bekommt die Trackable-ID als hidden input, der User füllt nur noch Restparameter (Schwellwert/topK/etc.) aus. Wenn das Mode noch keine Trackables hat, zeigt Step 1 einen Warnhinweis mit Verweis auf die Events-Section.
- **REQ-UI-073** **Phase 28 Renn- & Count-Bracket-Templates.** Fünf zusätzliche Templates in `templates.ts`: `finish_first` ("Wer kommt zuerst ins Ziel?", entity-scope, Schwelle N standardmäßig 1, baut auf race-Engine), `finish_last` ("Wer kommt als Letztes?", entity-scope, baut auf rank-asc), `count_zero` ("Passiert X gar nicht?", binär, compare eq 0), `count_less_than` (compare lt N) und `count_more_than` (compare gt N) für die typischen Renn-Schmerzpunkte (Sieger / Verlierer / Bracket-Wetten 0/<N/>N). `personalizedTitle()` und `TPL_ICONS` decken die neuen Templates ab.
- **REQ-UI-074** **Phase 29a Pins oben/unten (Top-to-Bottom Flow).** Da der Graph konzeptionell von oben (Eingang) nach unten (Ausgang) verläuft, sitzen Pins jetzt auf den **horizontalen Kanten** statt links/rechts: Input-Pins auf der **oberen** Kante, Output-Pins auf der **unteren** Kante. `pinX(idx, total)` verteilt sie gleichmäßig entlang `TILE_W` (margin 18, min 1 zentriert). `inputPinPos` retourniert `{x: o.x + pinX(...), y: o.y}`, `outputPinPos` retourniert `{x: o.x + pinX(...), y: o.y + TILE_H}`. Bezier-Wires haben vertikale Kontrollpunkte (`dy` statt `dx`). CSS: `.pin-in { top: -7px }`, `.pin-out { bottom: -7px }`. Pin-Popover platziert sich oberhalb/unterhalb statt links/rechts.
- **REQ-UI-075** **Phase 29b Inline Node Editor.** Die Tile-Body zeigt direkt ein **editierbares Control** für die primäre Property des Nodes (statt nur Read-only-Vorschau): `string`→`<input text>`, `number`→`<input number>`, `enum`→`<select>` mit `enumLabel()`, `modeRef:trackable`→`<select>` mit allen Mode-Trackables, `modeRef:entity`→`<select>` mit allen Default-Entities, `boolean`→`<input checkbox>`. Bei Nodes mit >1 Property wird zusätzlich ein `+N`-Badge angezeigt (Hinweis "weitere Eigenschaften im Inspektor"). `setNodeProp(node, key, value)` mutiert die Props direkt am Node ohne Selektion. `tile-body` stoppt `click`/`pointerdown`/`dragstart` Propagation, damit Editing nicht mit Node-Drag/Select kollidiert. Inspector-Drawer bleibt für komplexe Multi-Prop-Nodes verfügbar.
- **REQ-UI-076** **Phase 29c Farbige Entity/Event-Icons entfernt.** Die runden Farbbadges/Emoji-Avatare bei Entitäten und Events (Trackables) werden UI-weit entfernt — sie waren reine Dekoration, vom User nicht editierbar und führten zu Verwirrung. Betroffen: `ModeForm.svelte` (Entity- und Trackable-Reihen), `s/[id]/info/+page.svelte` (Lobby-Listen), `s/create/+page.svelte` (Default-Entity-Liste), `s/[id]/round/+page.svelte` (Entity-Buttons border-color, Trackable-Details Emoji), `modes/[id]/+page.svelte` und `modes/[id]/graphs/new/+page.svelte` (Trackable-Select-Options ohne Emoji-Präfix). Hidden inputs `entityColor`/`entityEmoji`/`trackableColor`/`trackableEmoji` bleiben für Server-Kompatibilität.
