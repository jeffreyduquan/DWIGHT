...
- **REQ-UI-021** Phase 7 ships the visual Blueprints-style editor (`src/lib/graph/GraphCanvas.svelte`): nodes auto-layout vertically by topological order; pins are coloured by `PIN_COLORS`; tap-to-connect (tap output pin → compatible inputs glow → tap input creates edge); inline per-node prop editor with `<select>`/`<input>` driven by `NodeSpec.props`; `+` FAB opens bottom-sheet palette grouped by `NODE_FAMILY`; SVG overlay draws curved edges between pin centres and supports tap-to-select / delete. JSON-textarea remains as `<details>` Advanced fallback.
